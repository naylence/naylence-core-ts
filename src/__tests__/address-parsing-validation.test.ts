...
describe('Address Parsing and Validation', () => {
  describe('FameAddress Class', () => {
    describe('constructor', () => {
      ...
      it('should throw on missing @ symbol', () => {
        expect(() => new FameAddress('invalid-address')).toThrow(
          "Missing '@' in address"
        );
      });
  ...
  describe('Participant Validation', () => {
    ...
    it('should reject invalid participant names', () => {
      expect(() => new FameAddress('user.name@/path')).toThrow(
        'Participant must match [A-Z a-z 0-9 _ -]+'
      );
      expect(() => new FameAddress('user@host@/path')).toThrow(
        'Participant must match [A-Z a-z 0-9 _ -]+'
      );
      expect(() => new FameAddress('user space@/path')).toThrow(
        'Participant must match [A-Z a-z 0-9 _ -]+'
      );
      expect(() => new FameAddress('user#@/path')).toThrow(
        'Participant must match [A-Z a-z 0-9 _ -]+'
      );
      expect(() => new FameAddress('user/name@/path')).toThrow(
        'Participant must match [A-Z a-z 0-9 _ -]+'
  ...
  describe('Host Validation', () => {
    it('should accept valid host names', () => {
      ...
      expect(
        () => new FameAddress('user@api-server.domain-name.org')
      ).not.toThrow();
    ...
    it('should reject wildcard in non-leftmost position', () => {
      expect(() => new FameAddress('user@example.*.com')).toThrow(
        "Wildcard '*' must be leftmost segment"
      );
      expect(() => new FameAddress('user@example.com.*')).toThrow(
        "Wildcard '*' must be leftmost segment"
      );
  ...
  describe('Path Validation', () => {
    ...
    it('should accept path with host+path format', () => {
      ...
      expect(
        () => new FameAddress('user@api.server.com/v1/endpoint')
      ).not.toThrow();
      expect(() => new FameAddress('user@example.com/')).not.toThrow();
    });

    it('should reject wildcards in paths', () => {
      expect(() => new FameAddress('user@/api/*/endpoint')).toThrow(
        'Wildcards not allowed in path segments'
      );
      expect(() => new FameAddress('user@/*/path')).toThrow(
        'Wildcards not allowed in path segments'
      );
      expect(() => new FameAddress('user@example.com/*/api')).toThrow(
        'Wildcards not allowed in path segments'
      );
    });
    ...
    it('should reject invalid path characters', () => {
      ...
      expect(() => new FameAddress('user@example.com/api#endpoint')).toThrow(
        'Bad segment'
      );
    });
  });
  ...
  describe('parseAddress Function', () => {
    ...
    it('should handle multiple @ symbols correctly', () => {
      ...
      expect(() => parseAddress('user@host@domain.com')).toThrow(
        'Participant must match'
      );
  ...
  describe('parseAddressComponents Function', () => {
    ...
    it('should handle path without leading slash in host+path format', () => {
      const [name, host, path] = parseAddressComponents(
  ...
  describe('formatAddress Function', () => {
    ...
    it('should validate participant in formatAddress', () => {
      expect(() => formatAddress('invalid.name', '/path')).toThrow(
        'Participant must match'
      );
    });
    ...
    it('should validate location in formatAddress', () => {
      ...
      expect(() => formatAddress('user', '/invalid*/path')).toThrow(
        'Bad segment'
      );
    });
  });
  ...
  describe('formatAddressFromComponents Function', () => {
    ...
    it('should throw when both host and path are null', () => {
      expect(() => formatAddressFromComponents('user', null, null)).toThrow(
    ...
    it('should validate participant', () => {
      expect(() =>
        formatAddressFromComponents('invalid.name', 'host.com', null)
      ).toThrow('Participant must match');
    });

    it('should validate host', () => {
      expect(() =>
        formatAddressFromComponents('user', 'invalid_host', null)
      ).toThrow('Bad host segment');
    });

    it('should validate path', () => {
      // Actual error message from validatePath uses "Bad segment" not "Wildcards not allowed"
      expect(() =>
        formatAddressFromComponents('user', null, '/invalid*/path')
      ).toThrow('Bad segment');
    });
  });
  ...
  describe('makeFameAddress Function', () => {
    ...
    it('should throw for invalid string', () => {
      expect(() => makeFameAddress('invalid')).toThrow(
        "Missing '@' in address"
      );
    });
  });
  ...
  describe('FameAddressSchema', () => {
    ...
    it('should reject invalid address strings', () => {
      ...
      expect(() =>
        FameAddressSchema.parse('invalid.participant@/path')
      ).toThrow();
    });
  ...
  describe('Edge Cases and Complex Scenarios', () => {
    it('should handle complex host names', () => {
      expect(
        () => new FameAddress('service@api-v2.sub-domain.example-site.org')
      ).not.toThrow();
      expect(() => new FameAddress('service@127.0.0.1')).not.toThrow();
      expect(
        () => new FameAddress('service@localhost.localdomain')
      ).not.toThrow();
    });

    it('should handle complex path structures', () => {
      expect(
        () => new FameAddress('api@example.com/v1/users/123/profile')
      ).not.toThrow();
      expect(
        () => new FameAddress('service@/api/v2.1/endpoint-name/action_type')
      ).not.toThrow();
    });
