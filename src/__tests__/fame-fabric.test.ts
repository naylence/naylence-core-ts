...
describe('Fame Fabric', () => {
  ...
  describe('FameFabric lifecycle management', () => {
    ...
    describe('enter', () => {
      ...
      it('should throw error when re-entering same instance', async () => {
        ...
        await expect(fabric.enter()).rejects.toThrow(
          'Cannot re-enter the same FameFabric instance'
        );
      });
    ...
    describe('exit', () => {
      ...
      it('should re-throw original error when stop succeeds', async () => {
        ...
        await expect(fabric.exit(originalError)).rejects.toThrow(
          'Original error'
        );
      });
  ...
  describe('sendMessage', () => {
    ...
    it('should send message with string address', async () => {
      const result = await fabric.sendMessage('test@mock.host', {
        message: 'hello',
      });
  ...
  describe('Factory methods', () => {
    describe('create', () => {
      ...
      it('should use the default config resolver when provided', async () => {
        ...
        const [normalizedRootConfig, rawRootConfig] =
          registeredFactory.lastArgs;
    ...
    describe('fromConfig', () => {
      it('should create a fabric from an explicit config', async () => {
        const fabric = await FameFabric.fromConfig({
          type: 'mock',
          opts: { nested: true },
        });
      ...
      it('should throw when config lacks a type', async () => {
        await expect(FameFabric.fromConfig({})).rejects.toThrow(
          "Configuration must have a 'type' field"
        );
      });

      it('should throw when config is not an object', async () => {
        await expect(
          FameFabric.fromConfig('bad' as unknown as Record<string, unknown>)
        ).rejects.toThrow(
      ...
      it('should forward factory arguments', async () => {
        await FameFabric.fromConfig(
          { type: 'mock' },
          { factoryArgs: ['beta', { flag: true }] }
        );
    ...
    describe('getOrCreate', () => {
      ...
      it('should pass options through when creating a new fabric', async () => {
        const options = {
          rootConfig: { fabric: { type: 'mock', opts: { custom: 42 } } },
        };
