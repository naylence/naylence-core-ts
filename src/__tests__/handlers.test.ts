...
import { createFameEnvelope, FameEnvelope } from "../naylence/fame/core/protocol/envelope";
import { localDeliveryContext, FameDeliveryContext } from "../naylence/fame/core/protocol/delivery-context";
...
describe("Handlers", () => {
  ...
  describe("Handler Type Usage Examples", () => {
    describe("FameMessageHandler", () => {
      ...
      it("should handle message and return FameMessageResponse", async () => {
        ...
        expect(result).toEqual({ envelope: mockEnvelope, context: mockContext });
      });
    ...
    describe("FameEnvelopeHandler", () => {
      ...
      it("should handle envelope with context", async () => {
        const handler: FameEnvelopeHandler = async (envelope: FameEnvelope, context?: FameDeliveryContext) => {
    ...
    describe("FameRPCHandler", () => {
      it("should handle RPC call and return direct result", async () => {
        const handler: FameRPCHandler = async (method: string, params?: Record<string, any>) => {
          return { method, params, result: "success" };
        };
        
        const result = await handler("test.method", { arg1: "value1" });
        expect(result).toEqual({ method: "test.method", params: { arg1: "value1" }, result: "success" });
      });
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle response with extra properties", () => {
      ...
      expect(normalizeHandlerResponse(responseWithExtras)).toBe(responseWithExtras);
    });
  });
});