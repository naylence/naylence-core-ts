  ...
  describe("ConnectorStateHelper", () => {
    describe("isActive", () => {
      ...
      it("should return false for non-STARTED states", () => {
        expect(new ConnectorStateHelper(ConnectorState.UNKNOWN).isActive).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.INITIALIZED).isActive).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.STOPPED).isActive).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.CLOSED).isActive).toBe(false);
      });
    });

    describe("isInactive", () => {
      it("should return true for STOPPED and CLOSED states", () => {
        expect(new ConnectorStateHelper(ConnectorState.STOPPED).isInactive).toBe(true);
        expect(new ConnectorStateHelper(ConnectorState.CLOSED).isInactive).toBe(true);
      });

      it("should return false for active states", () => {
        expect(new ConnectorStateHelper(ConnectorState.UNKNOWN).isInactive).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.INITIALIZED).isInactive).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.STARTED).isInactive).toBe(false);
      });
    });

    describe("canStart", () => {
      it("should return true for INITIALIZED and STOPPED states", () => {
        expect(new ConnectorStateHelper(ConnectorState.INITIALIZED).canStart).toBe(true);
        expect(new ConnectorStateHelper(ConnectorState.STOPPED).canStart).toBe(true);
      });

      it("should return false for other states", () => {
        expect(new ConnectorStateHelper(ConnectorState.UNKNOWN).canStart).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.STARTED).canStart).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.CLOSED).canStart).toBe(false);
      });
    });

    describe("canStop", () => {
      it("should return true for STARTED state", () => {
        expect(new ConnectorStateHelper(ConnectorState.STARTED).canStop).toBe(true);
      });

      it("should return false for other states", () => {
        expect(new ConnectorStateHelper(ConnectorState.UNKNOWN).canStop).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.INITIALIZED).canStop).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.STOPPED).canStop).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.CLOSED).canStop).toBe(false);
      });
    });

    describe("canClose", () => {
      it("should return true for INITIALIZED, STARTED, and STOPPED states", () => {
        expect(new ConnectorStateHelper(ConnectorState.INITIALIZED).canClose).toBe(true);
        expect(new ConnectorStateHelper(ConnectorState.STARTED).canClose).toBe(true);
        expect(new ConnectorStateHelper(ConnectorState.STOPPED).canClose).toBe(true);
      });

      it("should return false for UNKNOWN and CLOSED states", () => {
        expect(new ConnectorStateHelper(ConnectorState.UNKNOWN).canClose).toBe(false);
        expect(new ConnectorStateHelper(ConnectorState.CLOSED).canClose).toBe(false);
      });
    });
  ...
  describe("ConnectorStateUtils", () => {
    describe("utility functions", () => {
      it("should check isActive correctly", () => {
        ...
        expect(ConnectorStateUtils.isActive(ConnectorState.STOPPED)).toBe(false);
      });

      it("should check isInactive correctly", () => {
        expect(ConnectorStateUtils.isInactive(ConnectorState.STOPPED)).toBe(true);
        expect(ConnectorStateUtils.isInactive(ConnectorState.CLOSED)).toBe(true);
        expect(ConnectorStateUtils.isInactive(ConnectorState.STARTED)).toBe(false);
      });

      it("should check canStart correctly", () => {
        expect(ConnectorStateUtils.canStart(ConnectorState.INITIALIZED)).toBe(true);
        expect(ConnectorStateUtils.canStart(ConnectorState.STOPPED)).toBe(true);
        expect(ConnectorStateUtils.canStart(ConnectorState.STARTED)).toBe(false);
      });
      ...
      it("should check canClose correctly", () => {
        expect(ConnectorStateUtils.canClose(ConnectorState.INITIALIZED)).toBe(true);
        ...
        expect(ConnectorStateUtils.canClose(ConnectorState.UNKNOWN)).toBe(false);
      ...
      it("should parse valid state strings", () => {
        expect(ConnectorStateUtils.fromString("unknown")).toBe(ConnectorState.UNKNOWN);
        expect(ConnectorStateUtils.fromString("initialized")).toBe(ConnectorState.INITIALIZED);
        expect(ConnectorStateUtils.fromString("started")).toBe(ConnectorState.STARTED);
        expect(ConnectorStateUtils.fromString("stopped")).toBe(ConnectorState.STOPPED);
        expect(ConnectorStateUtils.fromString("closed")).toBe(ConnectorState.CLOSED);
      });
...
describe("BaseFameConnector", () => {
  class TestConnector extends BaseFameConnector {
    ...
    protected async onHandlerReplaced(handler: FameEnvelopeHandler): Promise<void> {
      // Test implementation
    }
    ...
    async pushToReceive(rawOrEnvelope: Uint8Array | FameEnvelope | FameChannelMessage): Promise<void> {
      // Test implementation
    }
  }
  ...
  describe("start", () => {
    ...
    it("should throw error if not in valid state to start", async () => {
      ...
      await expect(connector.start(mockHandler)).rejects.toThrow("Cannot start connector from state: started");
    });
  ...
  describe("stop", () => {
    ...
    it("should throw error if not in STARTED state", async () => {
      ...
      await expect(connector.stop()).rejects.toThrow("Cannot stop connector from state: stopped");
    });
  });
  ...
  describe("replaceHandler", () => {
    beforeEach(async () => {
      (connector as any)._state = ConnectorState.INITIALIZED;
      jest.spyOn(connector as any, "onStart").mockResolvedValue(undefined);
      jest.spyOn(connector as any, "onHandlerReplaced").mockResolvedValue(undefined);
      await connector.start(mockHandler);
    });
...
describe("Channel interfaces", () => {
  ...
  describe("ReadChannel", () => {
    ...
    it("should receive messages with timeout", async () => {
      ...
      expect(message).toEqual({ id: "message-1", data: "test message", timeout: 5000 });
    });

    it("should receive messages without timeout", async () => {
      const message = await channel.receive();
      expect(message).toEqual({ id: "message-1", data: "test message", timeout: undefined });
    });
  ...
  describe("ReadWriteChannel", () => {
    ...
    it("should support both read and write operations", async () => {
      ...
      expect(message).toEqual({ id: "message-1", data: "test message", timeout: 1000 });
