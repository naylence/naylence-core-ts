...
describe("DataFrameSchema", () => {
  ...
  it("should reject nonce with incorrect length", () => {
    ...
    expect(() => DataFrameSchema.parse(frame)).toThrow("Nonce must be exactly 12 bytes");
  });
...
describe("SecureOpenFrameSchema", () => {
  ...
  it("should reject ephemeral key with wrong length", () => {
    ...
    expect(() => SecureOpenFrameSchema.parse(frame)).toThrow("Ephemeral public key must be exactly 32 bytes");
  });
  ...
  it("should reject invalid base64 ephemeral key", () => {
    ...
    expect(() => SecureOpenFrameSchema.parse(frame)).toThrow("Invalid base64 ephemeral public key");
  });
});
...
describe("SecureAcceptFrameSchema", () => {
  ...
  it("should reject invalid ephemeral key length", () => {
    ...
    expect(() => SecureAcceptFrameSchema.parse(frame)).toThrow("Ephemeral public key must be exactly 32 bytes");
  });
});
...
describe("FameFrameUnionSchema", () => {
  ...
  it("should discriminate all frame types correctly", () => {
    ...
    frameTypes.forEach(type => {
      ...
      if (type === "AddressBind" || type === "AddressBindAck" || 
          type === "AddressUnbind" || type === "AddressUnbindAck" ||
          type === "CapabilityAdvertise" || type === "CapabilityAdvertiseAck" ||
          type === "CapabilityWithdraw" || type === "CapabilityWithdrawAck") {
        frame.address = "test@localhost/test";
      }
      
      if (type === "CapabilityAdvertise" || type === "CapabilityAdvertiseAck" ||
          type === "CapabilityWithdraw" || type === "CapabilityWithdrawAck") {
      ...
      if (type === "NodeHello" || type === "NodeWelcome" || type === "NodeAttach") {
