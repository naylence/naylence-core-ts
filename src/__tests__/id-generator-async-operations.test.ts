...
describe("ID Generator Async Operations", () => {
  ...
  describe("Fingerprint mode - Node.js simulation", () => {
      ...
      (globalThis as any).require = (name: string) => {
        if (name === "crypto") {
          return {
            createHash: (algorithm: string) => ({
              update: (data: Uint8Array) => {},
              digest: () => new Uint8Array([100, 101, 102, 103, 104, 105, 106, 107])
            })
          };
        }
        return undefined;
            ...
            createHash: (algorithm: string) => ({
              ...
              digest: () => {
                ...
                if (hashCallCount === 1) {
                  ...
                  return new Uint8Array([200, 201, 202, 203, 204, 205, 206, 207]);
                }
              }
            })
          };
    ...
    it("should throw error when no material provided in fingerprint mode sync", () => {
      ...
      }).toThrow("Fingerprint mode requires async operation in browser - use generateIdAsync instead");
    });
  ...
  describe("Crypto fallbacks", () => {
    it("should use Math.random fallback when no crypto available", () => {
      ...
      if (originalCrypto && originalCryptoGetRandomValues) {
        (globalThis as any).crypto = { getRandomValues: originalCryptoGetRandomValues };
      }
    });
      ...
      (globalThis as any).require = (name: string) => {
        if (name === "crypto") {
          return {
            createHash: (algorithm: string) => ({
              update: (data: Uint8Array) => {},
              digest: () => new Uint8Array([algorithm === "sha1" ? 1 : 2, 3, 4, 5])
            })
          };
        }
        return undefined;
  ...
  describe("getCanonicalArgv", () => {
    ...
    it("should handle errors in getCanonicalArgv", async () => {
      (globalThis as any).process = {
        get argv() { throw new Error("Process error"); },
        env: {}
      };
  ...
  describe("Consistency checks", () => {
    ...
    it("should produce different fingerprints for different inputs", async () => {
      const id1 = await generateIdAsync({ mode: "fingerprint", material: "material1" });
      const id2 = await generateIdAsync({ mode: "fingerprint", material: "material2" });
