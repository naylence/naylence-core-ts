...
describe("ID Generator Environment Detection", () => {
  ...
  it("should test blacklist collision handling (lines 427-428)", async () => {
    ...
    const id1 = await generateIdAsync({ length: 8, mode: "fingerprint", hashAlg: "SHA-256" });
    const id2 = await generateIdAsync({ length: 8, mode: "fingerprint", hashAlg: "SHA-256" });
  ...
  it("should force blacklist collision to test while loop", async () => {
    ...
    const promises = [];
    for (let i = 4; i <= 12; i++) {
      promises.push(generateIdAsync({ length: i, mode: "random", hashAlg: "SHA-256" }));
    }
